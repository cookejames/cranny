...
Cranny is a puzzle game. Each round, place nine polyomino pieces on a 6×6 grid with 7 randomly blocked squares. Modes: solo against the clock, and multiplayer (local development only until the Ably adapter lands): ad-hoc rooms where every player gets the same grid each round and scores by finishing order. The feature is called "multiplayer" everywhere (packages, routes, storage keys, UI), not "race".
