...
- Deploy: `scripts/deploy.sh` (lint, typecheck, test, build, deploy the rooms Lambda, upload, invalidate). Needs AWS credentials and an applied `infra/`. The Ably key lives only in SSM: set it with `aws ssm put-parameter --name /cranny/ably-key --type SecureString --overwrite --value '<key>'` (Terraform owns the parameter but ignores its value).
