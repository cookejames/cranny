...
- A room lives while anyone is in it. Once it is empty, its name can be used again and its scores are gone (§7). There is no maximum lifetime (§15).
...
- The adapter is chosen at build time by `VITE_ROOM_TRANSPORT` (`local` or `ably`), in one module (`src/net/adapters.ts`). `pnpm dev` uses `local` (from `apps/web/.env.development`), and the build fails on any other value. Production uses `ably` (from `apps/web/.env.production`, T6.2); an unset value hides the Multiplayer entry.
...
- **Rate limits:** API Gateway throttles the whole API (10 requests a second, bursts of 20), which caps the cost and how fast anyone can guess names. There is no per-IP limit: that would need AWS WAF, which isn't worth its cost for a casual game (decided in Phase 5).
- **Implementations:**
...
- While `VITE_ROOM_TRANSPORT` is unset, the button and both routes are absent: they redirect to `/`. The multiplayer screens are loaded on first use (`React.lazy`), so solo play never downloads them.
...
- **Knowing the name is the key.** Generated names have about 31 bits of entropy, and custom names are weaker; the UI says so (§4). The directory's overall rate limit slows guessing.
